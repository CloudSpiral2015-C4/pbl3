 ...
 *
 */
public class HourlySalesGetter {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job.setJarByClass(HourlySalesGetter.class);       // ★このファイルのメインクラスの名前
		job.setMapperClass(MyMapper.class);
		job.setReducerClass(MyReducer.class);
		job.setJobName("2015012");                        // ★自分の学籍番号
		...
		String outputpath = "out/HourlySales";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// valueとなる商品名を取得
			String name = csv[PosUtils.ITEM_NAME] + "\t" + csv[PosUtils.HOUR];
