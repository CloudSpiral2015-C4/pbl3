 ...
 *
 */
public class OnigiriCountByType {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		String outputpath = "out/onigiriCountByType";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String type = "etc";
			// ★ 梅おにぎり発見（商品名に梅っぽい名前を含む）
			if (csv[PosUtils.ITEM_NAME].matches(".*(梅|(うめ)|(ウメ)|(しそ)|(紫蘇)).*")) {
				type = "ume";
			}
			// 昆布おにぎり発見（商品名に昆布っぽい名前を含む）
			if (csv[PosUtils.ITEM_NAME].matches(".*(昆布|(こんぶ)|(コンブ)).*")) {
				type = "kombu";
			}
