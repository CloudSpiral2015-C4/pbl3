 ...
 *
 */
public class OnigiriSalesByDay {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job.setJarByClass(OnigiriSalesByDay.class);       // ★このファイルのメインクラスの名前
		...
		String outputpath = "out/onigiriSalesByDay";     // ★MRの出力先
		if (args.length > 0) {
			inputpath = args[0];
		}
	...
	public static class MyMapper extends Mapper<LongWritable, Text, CSKV, CSKV> {
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// keyとなる日付yymmddを作る
			String yymmdd = csv[PosUtils.YEAR] + csv[PosUtils.MONTH] + csv[PosUtils.DATE];

			// valueとなる売り上げ額を計算する
			int price = Integer.parseInt(csv[PosUtils.ITEM_COUNT]) * Integer.parseInt(csv[PosUtils.ITEM_PRICE]);

			// emitする
			context.write(new CSKV(yymmdd), new CSKV(price));
		}
	}
